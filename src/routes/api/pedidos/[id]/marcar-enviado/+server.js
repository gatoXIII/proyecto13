...
// ✅ VERSIÓN CORREGIDA - Valida datos del request, no del pedido actual
...
export async function POST({ params, request }) {
  ...
  try {
    ...
    // ✅ VALIDAR TRANSICIÓN (excluyendo validación de guia_envio que aún no existe en BD)
    const permitidas = [ESTADOS.PREPARANDO, ESTADOS.PAGADO];
    if (!permitidas.includes(pedido.estado)) {
      return json({ 
        success: false, 
        error: `No se puede marcar como enviado desde el estado "${pedido.estado}"` 
      }, { status: 400 });
    }
