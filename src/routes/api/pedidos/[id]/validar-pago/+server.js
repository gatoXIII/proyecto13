  ...
  try {
    ...
    if (aprobado) {
      ...
      updateData = {
        ...
        editable: true,
        fecha_pagado: null, // ✅ AGREGAR ESTA LÍNEA - Limpiar fecha de pago
        validado_por: null // ✅ AGREGAR ESTA LÍNEA - Limpiar validador
      };
      
      mensajeHistorial = `Pago rechazado por ${validado_por || 'Admin'}: ${motivo_rechazo}`;
      tipoNotificacion = 'pago_rechazado';

      metadataNotificacion = {
        motivo: motivo_rechazo,
        requiere_comprobante: motivo_rechazo.toLowerCase().includes('comprobante') || 
                              motivo_rechazo.toLowerCase().includes('pago') ||
                              motivo_rechazo.toLowerCase().includes('transferencia'),
        requiere_direccion: motivo_rechazo.toLowerCase().includes('dirección') || 
                          motivo_rechazo.toLowerCase().includes('direccion') ||
                          motivo_rechazo.toLowerCase().includes('domicilio')
      };
    ...
    try {
      await encolarNotificacion({
        ...
        prioridad: 'alta',
        metadata: aprobado ? {} : metadataNotificacion
      });
